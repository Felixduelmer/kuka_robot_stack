...
void PluginController::onReadPosesClick() {                       //
  ...
  std::ifstream scan_poses_file (scan_trajectory_file_path);   //读路径文件
  // the points in the trajectory txt file, read and written to the m_scanPointPoses vector
  if (scan_poses_file.is_open())
  {
    while ( getline (scan_poses_file,s) )   //输入流放进s
    {
      Eigen::VectorXd pointPose(7); //初始化
      int ind = 0;
      // parse the string
      std::string delimiter = " ";
      size_t pos = 0;
      std::string token;
      while ((pos = s.find(delimiter)) != std::string::npos) {  //std::string::npos长度参数
          token = s.substr(0, pos); //获得字符串s中从第0位开始的长度为pos的字符串(空格之前)
          pointPose(ind) =  std::stod(token);
          s.erase(0, pos + delimiter.length());
          ind += 1;
      }
      pointPose(ind) = std::stod(s);
      algorithm_->m_scanPointPoses.push_back(pointPose); //从向量后面插入
...
//      start_recording();
      algorithm_->setCurrentScanPointsIteration(0);
      algorithm_->VisionMovementFinishCallback();
    }
}
...
void PluginController::onClickedCreateSweepFromLabel() {
  ...
  auto sweep_path = sweep_dir + "sweeps";
  auto label_path = sweep_dir + "labels";
  std::vector<std::string> sweep_files; //字符串容器
  std::vector<std::string> label_files;
  char* path_sweep;
  char* path_label;
  path_sweep = &sweep_path[0];
...
void PluginController::onClickedpbtnSavePose() {
  auto pose = algorithm_->getCurrentRobotPose();
  std::ofstream out("/home/chenyu/MA/position.txt", std::ios_base::app);
  if (out.is_open())
  {
    out << std::to_string(pose.pose.position.x) << " " << std::to_string(pose.pose.position.y)
            << " " << std::to_string(pose.pose.position.z) << " " << std::to_string(pose.pose.orientation.x)
            << " " << std::to_string(pose.pose.orientation.y) << " " << std::to_string(pose.pose.orientation.z)
            << " " << std::to_string(pose.pose.orientation.w) << "\n";
     out.close();
...
void PluginController::onClickedpbtnRoutePlanning()
{
  algorithm_->onRoutePlanning();
  LOG_INFO("Route Planned!");
}

void PluginController::onClickedpbtnPrepareForScan()
{
  onReadPosesClick();
  onGoInitPosClick();
}

void PluginController::onClickedpbtnRunUSScan()
{
  onExecScanClick();
}


void PluginController::onClickedpbtnLoadSweepPosition()
{
  std::string sweep_dir = ui_->ledtSweepFilePath->text().toStdString();
  cout << sweep_dir << endl;
  DataList USImageData = readImage_IMF(sweep_dir);
  UltrasoundSweep* usSweep = static_cast<UltrasoundSweep*>(USImageData.getItem(0));
  std::ofstream out("/home/chenyu/MA/us_data/sweep_position2.txt", std::ios_base::app);
//  cout << usSweep->matrix().size() << endl;
//  cout << usSweep->matrix().size() << endl;
  SharedImageSet* sharedImage = USImageData.getImage(Data::UNKNOWN);
  int nLen = sharedImage->size();
  cout << nLen << endl;
  for(int i=0; i<nLen; i++)
    {
      ImFusion::mat4 sMatrix = usSweep->matrix(i);
      Eigen::Vector3d translation = sMatrix.block<3, 1>(0, 3);
      Eigen::Quaterniond quaternion(sMatrix.block<3, 3>(0, 0));
      out << translation.x() << " " << translation.y() << " " << translation.z() << " " << quaternion.x() << " " << quaternion.y() << " " << quaternion.z() << " " << quaternion.w() << "\n";
    }
}

void PluginController::onClickedpbtnLoadSweepImage()
{
  std::string file_path = ui_->ledtSweepFilePath->text().toStdString();
  ...
  for(int i = 0; i < nLen; i++) {
    MemImage* memImage = sharedImage->mem(i)->clone();
    memcpy(cv_imgIn.data, memImage->data(), memImage->byteSize());
    imwrite("/home/robotics-verse/projects/chenyu/0317_good_2/"+to_string(i)+".jpg", cv_imgIn);
  }
}

void PluginController::onClickedpbtnReplaceWithLabel()
{
  std::string file_path = ui_->ledtSweepFilePath->text().toStdString();
  DataList labelUSBefore = readImage_IMF(file_path);
  // DataList labelUSAfter = readImage_IMF(szFilePath, szFileName4);

  SharedImageSet* sharedImage = labelUSBefore.getImage(Data::UNKNOWN);
  int nLen = sharedImage->size();
...
//  std::ofstream out("/home/robotics-verse/projects/chenyu/calibration/time.txt", std::ios_base::app);
//  cout << sharedImage->timestamp(0) << endl;
//  for (int i=0; i<56; i++)
//  {
////    out.precision(0);
//    out.setf(ios::fixed,ios::floatfield);
//    out << sharedImage->timestamp(i) << "\n";
//  }
  for(int i=0;i<nLen;i++)
  {
    std::cout << "------------------------------------------ " << std::endl;
    cv::Mat img=cv::imread("/home/robotics-verse/projects/chenyu/0317_good_2_cropped/"+to_string(i)+".jpg");
    cv::Mat grayImg;
...
//    cout usSweepUncompensated->timestamp()
  }

  BackgroundExporter* sweepExporter = new BackgroundExporter();
  sweepExporter->save(usSweepUncompensated, std::string("/home/robotics-verse/projects/chenyu/0317_good_2_cropped.imf"));
}

void PluginController::onClickedpbtnExtractBoneSurface()
{
  std::string file_path = ui_->ledtSweepFilePath->text().toStdString();
  file_path = file_path.substr(0, file_path.length() - 4);
  for (int i=0; i<6895; i++)
  {
    string image_path = file_path + "/" + to_string(i) + ".jpg";
    Mat img = imread(image_path,0);
    Mat img_bone = algorithm_->onBoneExtraction(img);
//    LOG_INFO("hhhh");
    algorithm_->onSurfaceExtraction(img_bone,i);
  }
}

void PluginController::onClickedpbtnShowSurface()
{
  for (int i=0; i<6895; i++)
  {
    string txt_path = "/home/chenyu/MA/us_data/bone_surface1/" + to_string(i) + ".txt";
    ...
    for (int m=0;m<lineCnt*2;m++)
    {
    int p=m/2;
    int q=m%2;
    a[p][q]=b[m];
    }
    Mat img_surface = Mat::zeros(691, 710, CV_8UC1);
    for (int i=0;i<lineCnt;i++){
    img_surface.at<uchar>(a[i][1], a[i][0]) = 255;
    }
    LOG_INFO(i);
    namedWindow("输出图像-2", CV_WINDOW_AUTOSIZE);
    cv::imshow("输出图像-2",img_surface);
    cv::waitKey(5);
  }
}

void PluginController::onClickedpbtbGetUSPointCloud()
{
  algorithm_->onCoordinateTransformation();

//  pcl::PointCloud<pcl::PointXYZ> cloud;
//  cloud.points.resize (p_new_x.size());
//  for (size_t i = 0; i < cloud.points.size(); i++)
//  {
//    cloud.points[i].x = p_new_x[i];
//    cloud.points[i].y = p_new_y[i];
//    cloud.points[i].z = p_new_z[i];
//  }
//  pcl::io::savePLYFile ("/home/chenyu/MA/test_pcd.pcd", cloud);
