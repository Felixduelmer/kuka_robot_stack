...
void PluginAlgorithm::VisionMovementFinishCallback() {
  ...
  LOG_INFO("Current destination point : " + std::to_string(m_nIteration));
  if (m_nIteration < n_poseNumber && m_nIteration >= 0) {
    Eigen::Quaterniond quaPose(m_scanPointPoses[m_nIteration](6), m_scanPointPoses[m_nIteration](3), m_scanPointPoses[m_nIteration](4), m_scanPointPoses[m_nIteration](5)); //w x y z!!!
...
void PluginAlgorithm::onRoutePlanning()
{
  std::ifstream fin;
  fin.open("/home/chenyu/MA/position.txt", std::ios::in);
  double b[28];
  double a[4][7];
  int k;
  double str;
  int count = 0;
    ...
    a[p][q]=b[m];
      ...
      double euc_w_1, euc_w_2; // two width
      euc_w_1 = pow(pow(a[0][0] - a[1][0], 2) + pow(a[0][1] - a[1][1], 2), 0.5); //euclidean
      euc_w_2 = pow(pow(a[2][0] - a[3][0], 2) + pow(a[2][1] - a[3][1], 2), 0.5);
      double euc_w = euc_w_1 > euc_w_2 ? euc_w_1 : euc_w_2; //longer width
      int n = (euc_w-FP/2)/FP+1; //number of normal scan
      double z = (a[0][2]+a[1][2]+a[2][2]+a[3][2])/4; //z取平均值
      double theta1, theta2;
      theta1 = angle(a[0][0],a[0][1],a[1][0],a[1][1]);
      theta2 = angle(a[3][0],a[3][1],a[2][0],a[2][1]);

      std::ofstream fout ("/home/chenyu/MA/route.txt");
      fout << a[3][0] << " " << a[3][1] << " " << z <<  " " <<  a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] <<"\n";
      fout << a[0][0] << " " << a[0][1] << " " << z <<  " " <<  a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] << "\n"; //set point 1 directly as the next route

      for (k = 1; k < n; k++)
      {
          if (k % 2 == 1)
          {
              fout << a[0][0]+k*FP*cos(theta1) <<  " " << a[0][1]+k*FP*sin(theta1) << " " << z << " " << a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] << "\n";
              fout << a[3][0]+k*FP*cos(theta1) <<  " " << a[3][1]+k*FP*sin(theta1) << " " << z << " " << a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] << "\n";
          }
          else
          {
              fout << a[3][0]+k*FP*cos(theta1) <<  " " << a[3][1]+k*FP*sin(theta1) << " " << z <<  " " <<  a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] << "\n";
      ...
      if (n%2 == 0)
      {
          fout << a[2][0] <<  " " << a[2][1] << " " << z <<  " " <<  a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] << "\n";
          fout << a[1][0] <<  " " << a[1][1] << " " << z <<  " " <<  a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] << "\n";
      }
      else
      {
          fout << a[1][0] <<  " " << a[1][1] << " " << z <<  " " <<  a[3][3] << " " << a[3][4] << " " <<  a[3][5] << " " << a[3][6] << "\n";
...
cv::Mat PluginAlgorithm::onBoneExtraction(cv::Mat img) // get bone from US image
{
  int height = img.rows;
  int width = img.cols;
  for (int i=0; i<height; i++)
  {
    ...
    {
      if (img.at<uchar>(i,j) <= 140 || i >= 220 || i < 50)
      {
        img.at<uchar>(i,j) = 0;
      }
      else
  ...
  img_color = cv::Mat::zeros(img.size(), CV_8UC1);
  ...
  cv::Mat labels1, stats1, centroids1, img_color1;
  int nccomps1 = cv::connectedComponentsWithStats (
        img_color, //二值图像
        labels1,     //和原图一样大的标记图
        stats1, //nccomps×5的矩阵 表示每个连通区域的外接矩形和面积（pixel）
        centroids1 //nccomps×2的矩阵 表示每个连通区域的质心
        );


  //去除过小区域，初始化颜色表
  int colors1[nccomps1];
  colors1[0] = 0; // background pixels remain black.

  for(int i = 1; i < nccomps1; i++ ) {
        if (nccomps1 >= 3) {
          colors1[i] = 0;
          if( stats1.at<int>(i, cv::CC_STAT_AREA) > 1500 )
                colors1[i] = 255; // small regions are painted with black too.
        }
        else
        {
          colors1[0] = 0;
...
//  cout << nccomps1 << endl;
//  for(int i = 0; i < nccomps1; i++ ) {
//    cout << colors1[i] << " ";
//  }
//  cout << endl;

  //按照label值，对不同的连通域进行着色
  img_color1 = cv::Mat::zeros(img.size(), CV_8UC1);
  for( int y = 0; y < img_color1.rows; y++ )
        for( int x = 0; x < img_color1.cols; x++ )
        {
              int label1 = labels1.at<int>(y, x);
              CV_Assert(0 <= label1 && label1 <= nccomps1);
              img_color1.at<uchar>(y, x) = colors1[label1];
        }


  namedWindow("输出图像-2", CV_WINDOW_AUTOSIZE);
  cv::imshow("输出图像-2",img_color1);
  cv::waitKey(3);

  return img_color1;
}

void PluginAlgorithm::onSurfaceExtraction(cv::Mat img, int i)
{
  ofstream out("/home/chenyu/MA/us_data/bone_surface1/"+to_string(i)+".txt", std::ios_base::app);
//  LOG_INFO("qqqqqqqqq");
...
void PluginAlgorithm::onCoordinateTransformation()
{
  // 图像对于机器人的位置和姿态
  std::ifstream fin;
  fin.open("/home/chenyu/MA/us_data/sweep_position1.txt", std::ios::in);
  double b[6895*7];
  double a[6895][7];
  double str;
  int count = 0;
...
//  cout << count << endl;
  ...
  for (int i=0; i<6895; i++)
  {
    ...
    std::ifstream txtin;
    txtin.open(txt_path, std::ios::in);
...
//          cout << temp[0] << " " << temp[1] << " " << temp[2] << endl;
          ...
          // rotation matrix
          Eigen::Quaterniond q(a[i][6], a[i][3], a[i][4], a[i][5]);
          Eigen::Matrix3d rotationMatrix = q.toRotationMatrix();
//          LOG_INFO(rotationMatrix);
          // p
          Eigen::Vector3d p(temp[0]*FP1*1000/144,temp[2],temp[1]*50/652);
//          cout << temp[0] << " " << temp[0]*FP*1000/710 << endl;
//          LOG_INFO(p);
          // t
          Eigen::Vector3d translation(a[i][0],a[i][1],a[i][2]);
//          LOG_INFO(translation);
          // p_new
          Eigen::Vector3d p_new = rotationMatrix * p + translation; // p_new = R*p + t

          std::ofstream out("/home/chenyu/MA/p_new1.txt", std::ios_base::app);
//          out << temp[0] << " " << temp[1] << " " << temp[2] << "\n";
          out << std::to_string(p_new.x()) << " " << std::to_string(p_new.y())<< " " << std::to_string(p_new.z()) << "\n";
        }
    }
